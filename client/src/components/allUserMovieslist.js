...
import { getMovies, getMovieListBySearch } from '../redux/movies/moviesAction'

const GetMoviesListByUser = () => {
  ...
  useEffect(() => {
    dispatch(getMovies())
    setList(state.getMoviesList && state.getMoviesList.moviesList && state.getMoviesList.moviesList.data)
  }, [])

  const searchHandler = (e) => {
    e.preventDefault()
    dispatch(getMovieListBySearch(search))
    setSearchList(state.getSearchList && state.getSearchList.searchList && state.getSearchList.searchList.data)
    // setList(searchList)
  }
  return (<>
<div
...
>
  ...
  >
    ...
    <button className='btn btn-warning m-5' onClick={() => navigate("/moviesAdd")}>Add Movie</button>
    <div className='container'>
      <div className='col-md-6'>
        <div className='col-md-6'>
          <input type="text" onChange={e => setSearch(e.target.value)} placeholder='Search' className='form-control my-2 col-md-3' /><span><button onClick={e => searchHandler(e)} className='btn btn-dark'>Search</button></span>
        </div>
      </div>
                ...
                <div className="mt-5 col-md-4">
                  <div className="card shadow p-3 mb-5 bg-white rounded" style={{ width: "18rem" }}>
                    ...
                    <div className="card-body">
                      <h5 className="card-title">{item && item.movieName}</h5>
                      <p className="card-text">{item && item.description}</p>
                      <p className="card-text">{item && item.language}</p>
                    </div>
                    <div>
                      <button onClick={() => navigate(`/moviesList/${item._id}`)} className='btn btn-primary'>View</button>
                    </div>
                  </div>
                </div>
              </>
...
export default GetMoviesListByUser